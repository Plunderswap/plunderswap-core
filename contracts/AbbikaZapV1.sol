...
import {IAbbikaPair} from "./interfaces/IAbbikaPair.sol";
import {IAbbikaRouter02} from "./interfaces/IAbbikaRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by AbbikaSwap teams.
 */
contract AbbikaZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ZIL (WZIL)
    IWETH public WZIL;

    // AbbikaRouter interface
    IAbbikaRouter02 public abbikaRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from AbbikaSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address AbbikaRouter
    address private abbikaRouterAddress;

    // Address Wrapped ZIL (WZIL)
    address private WZILAddress;
    ...
    // tokenToZap = 0x00 address if ZIL
    event ZapIn(
    ...
    // token0ToZap = 0x00 address if ZIL
    ...
    // tokenToReceive = 0x00 address if ZIL
    event ZapOut(
        address indexed lpToken,
    ...
    /*
     * @notice Fallback for WZIL
     */
    receive() external payable {
        assert(msg.sender == WZILAddress);
    }

    /*
     * @notice Constructor
     * @param _WZILAddress: address of the WZIL contract
     * @param _abbikaRouter: address of the AbbikaRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WZILAddress,
        address _abbikaRouter,
        uint256 _maxZapReverseRatio
    ) {
        WZILAddress = _WZILAddress;
        WZIL = IWETH(_WZILAddress);
        abbikaRouterAddress = _abbikaRouter;
        abbikaRouter = IAbbikaRouter02(_abbikaRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ZIL in a WZIL pool (e.g. WZIL/token)
     * @param _lpToken: LP token address (e.g. ABB/ZIL)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ABB) to receive in the intermediary swap (e.g. ZIL --> ABB)
     */
    function zapInZIL(address _lpToken, uint256 _tokenAmountOutMin) external payable nonReentrant {
        WZIL.deposit{value: msg.value}();

        // Call zap function
        uint256 lpTokenAmountTransferred = _zapIn(WZILAddress, msg.value, _lpToken, _tokenAmountOutMin);
     ...
     * @param _lpToken: LP token address (e.g. ABB/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ABB) in the intermediary swap (e.g. BUSD --> ABB)
     */
    function zapInToken(
        address _tokenToZap,
    ...
    /*
     * @notice Zap 1 token and ZIL, rebalance them to 50-50, before adding them to LP
     * @param _token1ToZap: address of token1 to zap
     * @param _token1AmountIn: amount of token1 to zap
    ...
    /*
     * @notice Zap a LP token out to receive ZIL
     * @param _lpToken: LP token address (e.g. ABB/WZIL)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ZIL/WZIL) in the intermediary swap (e.g. ABB --> ZIL)
     */
    function zapOutZIL(
        address _lpToken,
        ...
        // Call zapOut
        uint256 tokenAmountToTransfer = _zapOut(_lpToken, WZILAddress, _tokenAmountOutMin);

        // Unwrap ZIL
        WZIL.withdraw(tokenAmountToTransfer);

        // Transfer ZIL to the msg.sender
        (bool success, ) = msg.sender.call{value: tokenAmountToTransfer}(new bytes(0));
        require(success, "ZIL: transfer fail");
     ...
     * @param _lpToken: LP token address (e.g. ABB/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ABB or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ABB) in the intermediary swap (e.g. BUSD --> ABB)
     */
     ...
     * @dev Use WZIL for _tokenToZap (if ZIL is the input)
    ...
    {
        address token0 = IAbbikaPair(_lpToken).token0();
        address token1 = IAbbikaPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IAbbikaPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = abbikaRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = abbikaRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @dev Use WZIL for _token0ToZap (if ZIL is the input)
    ...
    {
        require(
            _token0ToZap == IAbbikaPair(_lpToken).token0() || _token0ToZap == IAbbikaPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IAbbikaPair(_lpToken).token0() || _token1ToZap == IAbbikaPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IAbbikaPair(_lpToken).token0()) {
    ...
    /*
     * @notice View the details for single zap
     * @dev Use WZIL for _tokenToReceive (if ZIL is the asset to be received)
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IAbbikaPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IAbbikaPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = abbikaRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = abbikaRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IAbbikaPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "AbbikaRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "AbbikaRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        abbikaRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), abbikaRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(abbikaRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = abbikaRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = abbikaRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = abbikaRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = abbikaRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = abbikaRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
