...
import { IPlunderPair } from "./interfaces/IPlunderPair.sol";
import { IPlunderRouter02 } from "./interfaces/IPlunderRouter02.sol";
import { IWETH } from "./interfaces/IWETH.sol";
import { Babylonian } from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by PlunderSwap teams.
 */
contract PlunderZapV1 is Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;

  // Interface for Wrapped ZIL (WZIL)
  IWETH public WZIL;

  // PlunderRouter interface
  IPlunderRouter02 public plunderRouter;

  // Maximum integer (used for managing allowance)
  uint256 public constant MAX_INT = 2 ** 256 - 1;

  // Minimum amount for a swap (derived from PlunderSwap)
  uint256 public constant MINIMUM_AMOUNT = 1000;

  // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
  uint256 public maxZapReverseRatio;

  // Address PlunderRouter
  address private plunderRouterAddress;
   ...
   * @param _plunderRouter: address of the PlunderRouter
   * @param _maxZapReverseRatio: maximum zap ratio
   */
  constructor(
    address _WZILAddress,
    address _plunderRouter,
    uint256 _maxZapReverseRatio
  ) {
    WZILAddress = _WZILAddress;
    WZIL = IWETH(_WZILAddress);
    plunderRouterAddress = _plunderRouter;
    plunderRouter = IPlunderRouter02(_plunderRouter);
  ...
  {
    address token0 = IPlunderPair(_lpToken).token0();
    address token1 = IPlunderPair(_lpToken).token1();
    ...
    (uint256 reserveA, uint256 reserveB, ) = IPlunderPair(_lpToken)
      .getReserves();

    if (token0 == _tokenToZap) {
      swapTokenOut = token1;
      swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
      swapAmountOut = plunderRouter.getAmountOut(
        swapAmountIn,
        reserveA,
        reserveB
      );
  ...
  {
    require(
      _token0ToZap == IPlunderPair(_lpToken).token0() ||
        _token0ToZap == IPlunderPair(_lpToken).token1(),
      "Zap: Wrong token0"
    );
    require(
      _token1ToZap == IPlunderPair(_lpToken).token0() ||
        _token1ToZap == IPlunderPair(_lpToken).token1(),
      "Zap: Wrong token1"
    );
    ...
    if (_token0ToZap == IPlunderPair(_lpToken).token0()) {
      sellToken0 = (_token0AmountIn * reserveB > _token1AmountIn * reserveA)
        ? true
        : false;
      ...
      sellToken0 = (_token0AmountIn * reserveA > _token1AmountIn * reserveB)
        ? true
        : false;
      // Calculate the amount that is expected to be swapped
      swapAmountIn = _calculateAmountToSwapForRebalancing(
    ...
    returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut)
  {
    ...
    if (token1 == _tokenToReceive) {
      ...
      uint256 tokenAmountIn = (_lpTokenAmount * reserveA) /
        IPlunderPair(_lpToken).totalSupply();
  ...
  function _zapIn(
    ...
    uint256[] memory swapedAmounts = plunderRouter.swapExactTokensForTokens(
      swapAmountIn,
      _tokenAmountOutMin,
      path,
      address(this),
    ...
    (, , lpTokenReceived) = plunderRouter.addLiquidity(
      path[0],
      path[1],
  ...
  function _zapOut(
    ...
    (uint256 amount0, uint256 amount1) = IPlunderPair(_lpToken).burn(
      address(this)
    );

    require(amount0 >= MINIMUM_AMOUNT, "plunderRouter: INSUFFICIENT_A_AMOUNT");
    require(amount1 >= MINIMUM_AMOUNT, "plunderRouter: INSUFFICIENT_B_AMOUNT");
    ...
    // Swap tokens
    plunderRouter.swapExactTokensForTokens(
      swapAmountIn,
      _tokenAmountOutMin,
      path,
      address(this),
  ...
  function _approveTokenIfNeeded(address _token) private {
    if (IERC20(_token).allowance(address(this), plunderRouterAddress) < 1e24) {
      // Re-approve
      IERC20(_token).safeApprove(plunderRouterAddress, MAX_INT);
    }
  }
  ...
  function _calculateAmountToSwap(
    ...
    uint256 nominator = plunderRouter.getAmountOut(
      halfToken0Amount,
      _reserve0,
      _reserve1
    );
    uint256 denominator = plunderRouter.quote(
      halfToken0Amount,
